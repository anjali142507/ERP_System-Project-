  ...
  const onSubmit = async (data) => {
    ...
    try {
      const orderData = {
        ...
        products: orderItems.map(item => `${item.productName}|${item.quantity}|${item.unitPrice}`),
      };

      if (editingOrder) {
        await API.put(`/api/purchase-orders/${editingOrder.id}`, orderData);
        ...
        <Table>
          <TableHead>
            <TableRow sx={{ background: "linear-gradient(90deg, #F2867A 0%, #EF6F62 100%)" }}>
              ...
              <TableCell sx={{ color: "#ffffff", fontWeight: 700, borderBottom: "2px solid #F3DAD5" }}>Order ID</TableCell>
              <TableCell sx={{ color: "#ffffff", fontWeight: 700, borderBottom: "2px solid #F3DAD5" }}>Supplier</TableCell>
              <TableCell sx={{ color: "#ffffff", fontWeight: 700, borderBottom: "2px solid #F3DAD5" }}>Expected Delivery</TableCell>
              <TableCell align="right" sx={{ color: "#ffffff", fontWeight: 700, borderBottom: "2px solid #F3DAD5" }}>Total Amount</TableCell>
              <TableCell sx={{ color: "#ffffff", fontWeight: 700, borderBottom: "2px solid #F3DAD5" }}>Status</TableCell>
              <TableCell align="center" sx={{ color: "#ffffff", fontWeight: 700, borderBottom: "2px solid #F3DAD5" }}>Actions</TableCell>
            </TableRow>
          </TableHead>
          <TableBody>
            {orders.length === 0 ? (
              <TableRow>
                <TableCell colSpan={6} align="center" sx={{ py: 4, color: "#BBAFA9" }}>No purchase orders found</TableCell>
              </TableRow>
            ) : (
              orders.map((order) => (
                <Fragment key={order.id}>
                  <TableRow hover>
                    ...
                    <TableCell sx={{ color: "#7A6D67" }}>{order.supplierName}</TableCell>
                    <TableCell sx={{ color: "#7A6D67" }}>{new Date(order.expectedDeliveryDate).toLocaleDateString()}</TableCell>
                    ...
                    <TableCell>
                      <Chip
                        ...
                        onClick={() => {
                          const statuses = ["Ordered", "Received"];
